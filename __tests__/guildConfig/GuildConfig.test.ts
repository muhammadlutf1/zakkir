...
test("resolve applies the guild config defaults", async () => {
	const store = memoryStore();
	await store.set({ guildId: "guild-1", ...savedConfig });
	const configs = new GuildConfig(store);
...
test("resolve prefers command options over the guild config", async () => {
	const store = memoryStore();
	await store.set({ guildId: "guild-1", ...savedConfig });
	const configs = new GuildConfig(store);
...
test("omitted option fields fall through to the guild config", async () => {
	const store = memoryStore();
	await store.set({ guildId: "guild-1", ...savedConfig });
	const configs = new GuildConfig(store);
...
test("resolve leaves defaults unresolved for a guild with no saved config", async () => {
	const configs = new GuildConfig(memoryStore());

...
test("plays directly when a single Rewayah covers and the reciter comes from the option", async () => {
...
test("resolves the reciter through the guild config", async () => {
...
test("resolves the reciter through the global defaults", async () => {
...
test("plays the resolved default Rewayah directly when it covers and is the only one", async () => {
	const store = memoryStore();
	await store.set({ guildId: "g-1", language: "ar", defaultReciter: 10, defaultRewayah: 10 });
	const config = new GuildConfig(store);
...
test("shows the picker when more than one Rewayah covers, defaulting to the resolved default", async () => {
	const store = memoryStore();
...
test("shows the picker when the default Rewayah does not cover, as unresolved not an error", async () => {
	const store = memoryStore();
...
test("the command reciter option is preferred over the guild default", async () => {
	const store = memoryStore();
	await store.set({ guildId: "g-1", language: "ar", defaultReciter: 10, defaultRewayah: 10 });
	const config = new GuildConfig(store);
...
test("returns an error when no reciter resolves anywhere", async () => {
...
test("returns an error when the reciter option is unknown", async () => {
...
test("returns an error when no Rewayah covers the Surah for the Reciter", async () => {
